; Wallpaper Changer - Inno Setup Script with Code Signing Support
...
; Uncomment and configure ONE of the following options to enable code signing:
...
; Option 2: Sign using certificate from Windows Certificate Store (by thumbprint - more reliable)
; SignTool=signtool sign /sha1 YOUR_CERT_THUMBPRINT /fd SHA256 /tr http://timestamp.digicert.com /td SHA256 /v $f

; Option 3: Sign using PFX file (for development/testing)
; SignTool=signtool sign /f "C:\path\to\certificate.pfx" /p YOUR_PASSWORD /fd SHA256 /tr http://timestamp.digicert.com /td SHA256 /v $f

; Option 4: Sign using environment variables (recommended for CI/CD)
...
; SignTool=signtool sign /n "$SIGN_CERT_SUBJECT$" /fd SHA256 /tr $SIGN_TIMESTAMP_URL$ /td SHA256 /v $f

; Notes:
; - signtool.exe is part of Windows SDK
; - Default location: C:\Program Files (x86)\Windows Kits\10\bin\[version]\x64\signtool.exe
; - Timestamping is REQUIRED - it keeps signatures valid after certificate expires
; - See CODE_SIGNING_GUIDE.md for detailed instructions
; ===== END CODE SIGNING CONFIGURATION =====
...
Source: "CODE_SIGNING_GUIDE.md"; DestDir: "{app}"; Flags: ignoreversion
...
Name: "{group}\Code Signing Guide"; Filename: "{app}\CODE_SIGNING_GUIDE.md"
...
begin
  ...
  begin
    WizardForm.FinishedLabel.Caption :=
      ...
      'Note: If you see Windows SmartScreen warnings, see the Code Signing Guide for solutions.' + #13#10#13#10 +
